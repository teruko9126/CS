...
{
    parser_lastslash = filename.rfind("/");
...
string Parser::Getparserstring(void)
{
    return parser_currentcommand;
}
...
{
    if (parser_currentcommand.substr(0, 3) == "add" || parser_currentcommand.substr(0, 3) == "sub" ||
        parser_currentcommand.substr(0, 3) == "neg" || parser_currentcommand.substr(0, 2) == "eq" ||
        parser_currentcommand.substr(0, 2) == "gt" || parser_currentcommand.substr(0, 2) == "lt" ||
        parser_currentcommand.substr(0, 3) == "and" || parser_currentcommand.substr(0, 3) == "or" ||
        parser_currentcommand.substr(0, 3) == "not")
        return C_ARITHMETIC;
...
string Parser::arg1(void)
{
    ...
    if (ct != C_RETURN)
    {
        ...
        if (ct == C_ARITHMETIC)
        {
            a1 = parser_currentcommand.substr(0, parser_currentcommand.size());
        }
        else
        {
